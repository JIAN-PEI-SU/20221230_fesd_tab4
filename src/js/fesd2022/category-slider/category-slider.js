...
export default class CategorySlider {
  ...
  moveActive(speed) {
    ...
    const prevOffsetLeft = prevItem ? activeItem.offsetLeft - self.el.clientWidth / 2 + activeItem.clientWidth / 2 : 0
    if (self.slidable) {
  ...
  update() {
    ...
    self.translateMax = -Math.floor(
      Math.abs(getSlideTotalW(self.el) - self.el.querySelector('.wrapper').getBoundingClientRect().width)
    )
    self.slidable =
